<?php
    ...
    if($_REQUEST && isset($_REQUEST['type'])){
        switch($_REQUEST['type']){
            #===========================
            case 'GET':
               $get_info = mysqli_fetch_object(mysqli_query($kunloc,"SELECT * FROM account WHERE id = '$id_user' "));
               if($get_info->username){
                    $data_thongbao = mysqli_query($kunloc,"SELECT * FROM lich_su_hoat_dong WHERE username = '".$get_info->username."' ORDER BY id DESC LIMIT 0,30");
                    while($echo = mysqli_fetch_object($data_thongbao)){ 
                       if($echo->uid != $id_user){
                         $get_info_noti = mysqli_fetch_object(mysqli_query($kunloc,"SELECT * FROM account WHERE id = '".$echo->uid."' "));
                         $JSON[] = array(
                            "id" => $get_info->id,
                            "uid" => $echo->uid,
                            ...
                            "tieude" => $echo->tieude,
                            "seen" => $echo->seen,
                            "noidung" => $echo->noidung,
                            "href" => $echo->url,
                            "time" => _ago($echo->time,$rcs = 0),
                            "avatar" => $get_info_noti->avatar,
                            "session_verify" => verify($get_info_noti->confirm_status)  
            ...
            break;
             ...
             case 'MESS':
                #4
                $data_thongbao = mysqli_query($kunloc,"SELECT * FROM inbox WHERE uid = '$id_user' ORDER BY time DESC LIMIT 0,30"); 
                #1
                $data_thongbao2 = mysqli_query($kunloc,"SELECT * FROM inbox WHERE uid2= '$id_user' ORDER BY time DESC LIMIT 0,30"); 
                if(mysqli_num_rows($data_thongbao)){
                    while($echo = mysqli_fetch_object($data_thongbao)){
                        $nguoigui = mysqli_fetch_object(mysqli_query($kunloc,"SELECT * FROM account WHERE id = '".$echo->uid2."' "));
                        $JSON[] = array(
                            "uid" => $echo->uid,
                            "text" => substr(trim($echo->text),0,100)."...", 
                            "time" => _ago($echo->time,$rcs = 0), 
                            "seen" => $echo->seen,
                            "username" => $nguoigui->username,
                            "fullname" => $nguoigui->fullname,
                            "id" => $nguoigui->id,
                            "avatar" => $nguoigui->avatar,
                            //"session_verify" => verify($nguoigui->confirm_status)    
                        );
                    }
                    echo(json_encode($JSON, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));

                }else if(mysqli_num_rows($data_thongbao2)){
                    while($echo = mysqli_fetch_object($data_thongbao2)){
                        $nguoigui = mysqli_fetch_object(mysqli_query($kunloc,"SELECT * FROM account WHERE id = '".$echo->uid."' ORDER BY seen DESC LIMIT 0,30"));
                        $JSON[] .= array(
                            "uid" => $nguoigui->id,
                            "text" => substr(trim($echo->text),0,100)."...", 
                            "time" => _ago($echo->time,$rcs = 0), 
